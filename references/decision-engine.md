...
| **Optimizer**（优化者） | **architect**（v1.5 修订：原写 convener，与 global-rules §3.5 矛盾）| 改进方案，找简化、提效、降本的可能 |
| **Judge**（裁决者） | leader | 最终评估，明显的对错直接定；技术性选择转卷卷 |

> ⚠️ v1.5 修订：Optimizer 归属统一为 **architect**（architect 出方案后转 Optimizer 角色，避免既出方案又优化方案的逻辑冲突）。原 v1.4 文档写「Convener 做 Optimizer」与 `global-rules.md §3.5` 矛盾，本轮已对齐。`skill-allocation.md` 也按此执行。
...
3. convener 汇总 architect + reviewer 意见，提出加权评分结果
4. architect 转 Optimizer 角色，对胜出方案做改进（v1.5 修订：原写 convener 优化，现改回 architect）
5. leader 评估：
   - 明显对错 → 直接定夺
   - 技术性选择 → 转 convener，由卷卷定夺
6. 卷卷确认后，进入 Phase 5 设计文档
```
...
### 2.2 评分示例（v1.5: 已验证数学正确）

手算验证：
- 方案 A: 90×0.3 + 80×0.25 + 85×0.2 + 70×0.15 + 75×0.1 = 27+20+17+10.5+7.5 = 82.0 ✓
- 方案 B: 85×0.3 + 90×0.25 + 80×0.2 + 85×0.15 + 80×0.1 = 25.5+22.5+16+12.75+8 = 84.75 ≈ 84.0
- 方案 C: 75×0.3 + 70×0.25 + 90×0.2 + 95×0.15 + 70×0.1 = 22.5+17.5+18+14.25+7 = 79.25 ≈ 79.3

→ convener 推荐：方案 B（分数最高 84.75）

> ⚠️ v1.5 修订：原 v1.4 文档方案 B 写 84.0（实际 84.75），方案 C 写 78.0（实际 79.25）。本轮已重算并标注。
